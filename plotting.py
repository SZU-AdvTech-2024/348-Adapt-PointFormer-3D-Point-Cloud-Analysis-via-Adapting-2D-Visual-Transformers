...
def draw_point(blue_cloud, red_cloud, i):
    ...
    colors = plt.cm.YlGnBu(norm(closest_distances))  # 使用蓝到红的渐变颜色
    # colors = plt.cm.Blues_r(norm(closest_distances))
    # 创建3D绘图
    fig = plt.figure()
    ...
    plt.savefig(f"3d_scatter_plot_{i}.pdf", format="pdf")
    # 显示图像
    plt.show()


def plotting(xyzs, xyzs2):
    xyzs = xyzs.cpu().numpy()
